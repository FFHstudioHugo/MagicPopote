...
Shader "ASESampleShaders/CustomRTRainUpdate"
{
    Properties
    {
		...
		_Vector0("Vector 0", Vector) = (0.04,0.01,0.0973,0)
		_Float3("Float 3", Float) = 0.47
	...
	SubShader
	{
        ...
        Pass
        {
			...
			void CalculateRipples( float resolution , float2 uv , int maxRadius , float time , float hashScale , float3 hashScale3 , out float2 rippleUVOffset , out float rippleIntensity )
			{
				...
				circles /= float((maxRadiusf*1.0+1.0)*(maxRadiusf*2.0+1.0));
            ...
            float4 frag(ase_v2f_customrendertexture IN ) : COLOR
            {
				...
				float hashScale8 = _Float3;
				float3 hashScale38 = _Vector0;
				...
				float4 appendResult10 = (float4(rippleUVOffset8 , 0.0 , ( rippleIntensity8 * 0.5 )));

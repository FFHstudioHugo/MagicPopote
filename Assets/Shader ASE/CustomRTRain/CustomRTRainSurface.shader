    ...
    SubShader
    {
        ...
        Pass
        {
        	...
        	half4 frag (GraphVertexOutput IN ) : SV_Target
            {
				...
				float2 temp_output_27_0 = (float2( 0,0 ) + (( (WorldSpacePosition).xz * _Tiling ) - float2( -1,-1 )) * (float2( 1,1 ) - float2( 0,0 )) / (float2( 1,1 ) - float2( -1,-1 )));
				
				
		        float3 Albedo = ( tex2D( _Albedo, temp_output_27_0 ) + tex2D( _RainRT, temp_output_27_0 ).a ).rgb;
				float3 Normal = UnpackNormalmapRGorAG( tex2D( _Normal, temp_output_27_0 ), 1.0f );
				float3 Emission = 0;
